...
class LifeTrackerViewController: UIViewController, TimeManager {
  ...
  override init(nibName nibNameOrNil: String?, bundle nibBundleOrNil: Bundle?) {
    let startGameState = GameLifeData(
      playerCount: .twoPlayer,
      poisonEnabled: false,
      p1LifeData: PlayerLifeData(life: startingLife2P, poison: startingPoison),
      p2LifeData: PlayerLifeData(life: startingLife2P, poison: startingPoison),
      p1Of4LifeData: PlayerLifeData(
        life: startingLife4P,
        poison: startingPoison
      ),
    ...
    self.lifeTrackerView = LifeTrackerView(
      gameLifeViewModel: self.gameLifeViewModel,
      timeManager: self,
    ...
    NotificationCenter.default.addObserver(
      ...
      name: NSNotification.Name(
        rawValue: GameNotifications.GameDidTapResetNotification
      ),
      object: nil
    )
  }
  ...
  @objc private func gameStateDidChangeNotification(
  ...
  @objc private func didTapResetNotification(_ notification: NSNotification) {
    let alert = UIAlertController(
      title: "Reset Game",
      message: "Are you sure you wish to reset the life totals?",
